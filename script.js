/* ================================================================
   ...
   Features: Login, RBAC, Dynamic Analysts, Break Overrides,
             ...
             Config-Driven Shift Engine, Full Task Reassignment
   ================================================================ */
"use strict";
...
const USERS = [
  ...
  { username: "agrima",  password: "5555", role: "analyst" },
...
// ============================================================
// ★ CENTRAL SHIFT CONFIG — Single Source of Truth
// ============================================================

// Default static definitions (fallback)
...
// Live config object — populated at boot, overridable
let SHIFT_CONFIG = {
  weekday:  [],
  saturday: [],
  sunday:   []
...
// Legacy alias — always mirrors SHIFT_CONFIG.weekday
let ANALYSTS = [];
...
// Computed backup chain from current weekday roster
function getBackupChain() {
  return SHIFT_CONFIG.weekday.map(a => a.id);
}
...
/** Returns the analyst list for a given date */
function getShiftConfig(date) {
  const d = (date || state.now).getDay();
  if (d === 6) return SHIFT_CONFIG.saturday;
  if (d === 0) return SHIFT_CONFIG.sunday;
  return SHIFT_CONFIG.weekday;
}

/** Persist config to localStorage */
function persistShiftConfig() {
  lsSet("shift_config", SHIFT_CONFIG);
  lsSet("analystColors", ANALYST_COLORS);
}

/** Trigger full recalculation after any config change */
function triggerFullRecalculation() {
  const analysts = getShiftConfig(state.now);
...
/** Update a single analyst's shift in a day-type */
function updateAnalystShift(analystId, newStart, newEnd, dayType) {
  dayType = dayType || "weekday";
  const list = SHIFT_CONFIG[dayType];
  if (!list) return;
  const analyst = list.find(a => a.id === analystId);
  if (!analyst) return;
  analyst.shiftStart = newStart;
  analyst.shiftEnd   = newEnd;
  // Mirror to ANALYSTS if weekday
  if (dayType === "weekday") {
    const wa = ANALYSTS.find(a => a.id === analystId);
    if (wa) { wa.shiftStart = newStart; wa.shiftEnd = newEnd; }
  }
  persistShiftConfig();
  triggerFullRecalculation();
}

/** Replace entire weekend config for a day type */
function setWeekendConfig(dayType, analystsArray) {
  if (!SHIFT_CONFIG[dayType]) return;
  SHIFT_CONFIG[dayType] = analystsArray;
  persistShiftConfig();
  triggerFullRecalculation();
}

/** Full reassignment engine — rebuilds all task owners */
function reassignAllTasks() {
  const workloadMap = {};
  state.taskLog.forEach(task => {
    const best = findBestAnalyst(task.time, task.type, workloadMap);
    if (!task.manualOwner) {
      task.actualOwner   = best;
      task.plannedOwner  = best;
      task.autoReassigned = true;
      if (best) workloadMap[best] = (workloadMap[best] || 0) + 1;
    }
  });
}

/** Score-based best analyst selection */
function findBestAnalyst(time, taskType, workloadMap) {
  const analysts = getShiftConfig(state.now);
  let best = null, bestScore = -999;
  analysts.forEach(a => {
    const score = scoreAnalyst(a.id, time, taskType, workloadMap, state.now);
...
function loadState() {
  ...
  // ★ Load SHIFT_CONFIG first (highest priority)
  const savedShift    = lsGet("shift_config");
  const savedAnalysts = lsGet("analysts");
  const savedColors   = lsGet("analystColors");

  if (savedShift && savedShift.weekday && savedShift.weekday.length) {
    SHIFT_CONFIG = savedShift;
  } else if (savedAnalysts && savedAnalysts.length) {
    // Migrate legacy analyst array into config
    SHIFT_CONFIG.weekday  = savedAnalysts;
    SHIFT_CONFIG.saturday = [...DEFAULT_SATURDAY];
    SHIFT_CONFIG.sunday   = [...DEFAULT_SUNDAY];
  } else {
    // Fresh install — use defaults
    SHIFT_CONFIG.weekday  = DEFAULT_WEEKDAY.map(a => ({ ...a }));
    SHIFT_CONFIG.saturday = DEFAULT_SATURDAY.map(a => ({ ...a }));
    SHIFT_CONFIG.sunday   = DEFAULT_SUNDAY.map(a => ({ ...a }));
  }

  // Keep ANALYSTS in sync with weekday config
  ANALYSTS = SHIFT_CONFIG.weekday;

  if (savedColors) ANALYST_COLORS = { ...ANALYST_COLORS, ...savedColors };

  // Ensure all analysts have colours
  ANALYSTS.forEach((a, i) => {
    if (!ANALYST_COLORS[a.id]) ANALYST_COLORS[a.id] = COLOR_PALETTE[i % COLOR_PALETTE.length];
  });
...
// ============================================================
// ★ ANALYST HELPERS — now use getShiftConfig()
// ============================================================

function getActiveAnalysts(date) {
  return getShiftConfig(date).map(a => ({ ...a }));
}

function getAnalystConfig(id, date) {
  return getShiftConfig(date).find(a => a.id === id) || null;
}

function getAnalystName(id) {
  if (!id) return "";
  // Search across all configs
  const all = [...SHIFT_CONFIG.weekday, ...SHIFT_CONFIG.saturday, ...SHIFT_CONFIG.sunday];
...
function getBackup(primaryId, time24, date, excludeIds) {
  ...
  const chain = getBackupChain().filter(id =>
    id !== primaryId &&
...
function recalculateAssignments() {
  // ★ Recalculate using current shift config
  const analysts = getShiftConfig(state.now);
  BREAK_DEFINITIONS = generateDynamicBreaks(analysts);
  state.taskLog = generateTaskSchedule(state.now);
  saveState();
...
function generateTaskSchedule(date) {
  date = date || state.now;
  // ★ Use getActiveAnalysts (which calls getShiftConfig internally)
  ...
  const shiftEndCap = analysts.length ? Math.max(...analysts.map(a => timeToMinutes(a.shiftEnd))) : timeToMinutes("23:30");
  ...
  const backupChain  = getBackupChain().filter(id => analysts.find(a => a.id === id));
  const firstAnalysts = analystIds.slice(0, 2);

  // STAFFING REPORTS
  const staffingTimes = isSun ? ["11:30"] : ["08:30","14:00","17:00"];
  staffingTimes.forEach(time => {
    if (timeToMinutes(time) <= shiftEndCap) {
      const owner = bestOwner(analystIds, time, "staffing");
  ...
  if (!isSat && !isSun && timeToMinutes("09:30") <= shiftEndCap) {
    const owner = bestOwner(firstAnalysts, "09:30", "ops") ||
                  bestOwner(analystIds,    "09:30", "ops");
  ...
  if (!isSat && !isSun) {
    for (let m = timeToMinutes("10:00"); m <= ciiCap; m += 30) {
      ...
      const owner = bestOwner(analystIds, time, "cii");
  ...
  for (let m = timeToMinutes("10:00"); m <= statusCap; m += 60) {
    const time  = minutesToTime(m);
    const owner = bestOwner(analystIds, time, "status");
    tasks.push(makeTask(time, "Status Report", owner, "status", date, false, !owner));
  }

  // EMAIL HANDLING — continuous
  const emailStart = analysts.length ? Math.min(...analysts.map(a => timeToMinutes(a.shiftStart))) : timeToMinutes("08:00");
  for (let m = emailStart; m < shiftEndCap; m += 30) {
    const time  = minutesToTime(m);
    const owner = bestOwner(backupChain.length ? backupChain : analystIds, time, "email");
...
function autoReassignTasks() {
  ...
  // ★ Use current day's shift config
  const allTodayAnalysts = getShiftConfig(state.now);
  const activeAnalysts   = allTodayAnalysts
    .filter(a =>
      state.ptoStatus[a.id] !== "pto" &&
...
function generateAIRecommendations() {
  ...
  const todayAnalysts = getShiftConfig(state.now);
  const activeNow = todayAnalysts.filter(a =>
...
function renderErrorCommandCenter() {
  ...
  body.innerHTML = open.map((err, i) => {
    ...
    return `<div class="ecc-row">
      <div class="ecc-left">
        <span class="ecc-sev ${sevClass}">${(err.severity||"MED").toUpperCase()}</span>
        <span class="ecc-analyst" style="color:${ANALYST_COLORS[err.analyst]||'var(--text-primary)'}">${getAnalystName(err.analyst)}</span>
      ...
      <div class="ecc-actions">
        ...
        <button class="btn-small" style="color:var(--warn);border-color:var(--warn)"
          onclick="openEscalateModal(${origIdx})">↑ ESC</button>
        <button class="btn-small" style="color:var(--danger);border-color:var(--danger)"
          onclick="deleteErrorByIdx(${origIdx})">🗑</button>
      </div>
    </div>`;
  }).join("");
}
...
function openEscalateModal(idx) {
  document.getElementById("escalate-error-id").value = idx;
...
function submitEscalation() {
  const idx  = parseInt(document.getElementById("escalate-error-id").value);
  ...
  if (!isNaN(idx) && state.errorLog[idx]) {
    ...
    state.errorLog[idx].escalatedTo = note;
...
function updateWeekendBanner() {
  ...
  if (d === 6) {
    ...
    const sat = SHIFT_CONFIG.saturday;
    text.textContent = sat.length
      ? "SATURDAY SCHEDULE: " + sat.map(a => `${a.name} (${a.shiftStart}–${a.shiftEnd})`).join(" · ")
      : "SATURDAY SCHEDULE: No analysts configured";
  } else if (d === 0) {
    banner.classList.remove("hidden");
    const sun = SHIFT_CONFIG.sunday;
    text.textContent = sun.length
      ? "SUNDAY SCHEDULE: " + sun.map(a => `${a.name} (${a.shiftStart}–${a.shiftEnd})`).join(" · ")
      : "SUNDAY SCHEDULE: No analysts configured";
  } else {
...
// ============================================================
// AGENT CARDS — uses ANALYSTS (weekday roster) for display
// ============================================================
...
function renderAgentCards() {
  ...
  // ★ Show today's active roster
  const todayRoster = getShiftConfig(state.now);

  const taskCounts = {}, doneCounts = {};
  todayRoster.forEach(a => { taskCounts[a.id] = 0; doneCounts[a.id] = 0; });
  state.taskLog.forEach(task => {
...
function renderChart() {
  ...
  todayRoster.forEach(a => { counts[a.id] = 0; assigned[a.id] = 0; });
  ...
  container.innerHTML = todayRoster.map(a => {
...
function populateScheduleFilters() {
  ...
  if (!sel) return;
  // ★ Populate from all shift configs (union of unique IDs)
  const allIds = new Map();
  [...SHIFT_CONFIG.weekday, ...SHIFT_CONFIG.saturday, ...SHIFT_CONFIG.sunday]
    .forEach(a => allIds.set(a.id, a.name));
  sel.innerHTML = `<option value="all">All Analysts</option>`;
  allIds.forEach((name, id) => { sel.innerHTML += `<option value="${id}">${name}</option>`; });
}
...
function renderPTOPanel() {
  ...
  if (!grid) return;
  // ★ Show all analysts across all configs
  const allAnalysts = [...SHIFT_CONFIG.weekday];
  // Add weekend-only analysts not in weekday
  [...SHIFT_CONFIG.saturday, ...SHIFT_CONFIG.sunday].forEach(a => {
    if (!allAnalysts.find(x => x.id === a.id)) allAnalysts.push(a);
  });
  grid.innerHTML = allAnalysts.map(cfg => {
...
function populateModalAnalysts(selectId) {
  ...
  if (!sel) return;
  const todayRoster = getShiftConfig(state.now);
  sel.innerHTML = `<option value="">Unassigned</option>${todayRoster.map(a =>
...
// ============================================================
// ★ ANALYST MANAGER — now edits SHIFT_CONFIG
// ============================================================
...
function renderAnalystMgrBody() {
  ...
  const tabs = ["weekday","saturday","sunday"];
  const labels = { weekday: "WEEKDAY ROSTER", saturday: "SATURDAY ROSTER", sunday: "SUNDAY ROSTER" };

  body.innerHTML = tabs.map(dayType => {
    const list = SHIFT_CONFIG[dayType];
    return `<div style="border-bottom:2px solid var(--border)">
      <div style="padding:10px 16px 6px;background:var(--bg-panel2);border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between">
        <span style="font-family:var(--font-mono);font-size:10px;color:var(--accent);letter-spacing:.15em">${labels[dayType]}</span>
        <button onclick="addAnalystToConfig('${dayType}')"
          style="background:rgba(0,200,255,0.1);color:var(--accent);border:1px solid var(--accent);border-radius:4px;padding:3px 10px;cursor:pointer;font-size:10px;font-family:var(--font-mono)">+ ADD</button>
      </div>
      <div style="display:grid;grid-template-columns:110px 80px 80px 80px 44px 1fr 36px;gap:6px;padding:6px 16px 4px;border-bottom:1px solid var(--border)">
        ${["NAME","START","END","ROLE","COLOR","",""].map(h => `<span style="font-family:var(--font-mono);font-size:9px;color:var(--muted);letter-spacing:.1em">${h}</span>`).join("")}
      </div>
      ${list.map((a, i) => `
        <div class="analyst-mgr-row">
          <input class="ctrl-input" value="${a.name}"
            onchange="SHIFT_CONFIG['${dayType}'][${i}].name=this.value;if('${dayType}'==='weekday'){const wa=ANALYSTS.find(x=>x.id===SHIFT_CONFIG['${dayType}'][${i}].id);if(wa)wa.name=this.value;}"
            style="width:100%">
          <input class="ctrl-input" type="time" value="${a.shiftStart}"
            onchange="SHIFT_CONFIG['${dayType}'][${i}].shiftStart=this.value;if('${dayType}'==='weekday'){const wa=ANALYSTS.find(x=>x.id===SHIFT_CONFIG['${dayType}'][${i}].id);if(wa)wa.shiftStart=this.value;}"
            style="width:100%">
          <input class="ctrl-input" type="time" value="${a.shiftEnd}"
            onchange="SHIFT_CONFIG['${dayType}'][${i}].shiftEnd=this.value;if('${dayType}'==='weekday'){const wa=ANALYSTS.find(x=>x.id===SHIFT_CONFIG['${dayType}'][${i}].id);if(wa)wa.shiftEnd=this.value;}"
            style="width:100%">
          <select class="ctrl-select" onchange="SHIFT_CONFIG['${dayType}'][${i}].role=this.value" style="width:100%">
          ...
          <span></span>
          <button onclick="removeAnalystFromConfig('${dayType}',${i})"
            style="background:rgba(255,59,59,0.1);color:var(--danger);border:1px solid var(--danger);border-radius:4px;padding:4px 8px;cursor:pointer;font-size:11px">🗑</button>
        </div>`).join("")}
    </div>`;
  }).join("");
}

function addAnalystToConfig(dayType) {
  const id = "analyst_" + genId();
  SHIFT_CONFIG[dayType].push({
    id, name: "New Analyst",
    shiftStart: "09:00", shiftEnd: "18:00",
    role: "RTA", isPTO: false
  });
  ANALYST_COLORS[id] = COLOR_PALETTE[SHIFT_CONFIG[dayType].length % COLOR_PALETTE.length];
  state.ptoStatus[id] = "present";
  if (dayType === "weekday") ANALYSTS = SHIFT_CONFIG.weekday;
  renderAnalystMgrBody();
}

function removeAnalystFromConfig(dayType, i) {
  const name = SHIFT_CONFIG[dayType][i]?.name || "analyst";
  if (!confirm(`Remove ${name} from ${dayType} roster?`)) return;
  SHIFT_CONFIG[dayType].splice(i, 1);
  if (dayType === "weekday") ANALYSTS = SHIFT_CONFIG.weekday;
  renderAnalystMgrBody();
}

// Legacy functions kept for compatibility
function addNewAnalyst() { addAnalystToConfig("weekday"); }
function removeAnalyst(i) { removeAnalystFromConfig("weekday", i); }

function closeAnalystManager() {
  closeAllModals();
  // ★ Sync ANALYSTS with weekday config
  ANALYSTS = SHIFT_CONFIG.weekday;
  BREAK_DEFINITIONS = generateDynamicBreaks(ANALYSTS);
  persistShiftConfig();
  recalculateAssignments();
...
function bootApp() {
  ...
  // Load and sync shift config
  loadState();

  // Ensure PTO state for all known analysts
  const allAnalysts = [
    ...SHIFT_CONFIG.weekday,
    ...SHIFT_CONFIG.saturday,
    ...SHIFT_CONFIG.sunday
  ];
  allAnalysts.forEach(a => {
    if (!state.ptoStatus[a.id]) state.ptoStatus[a.id] = "present";
  });

  BREAK_DEFINITIONS = generateDynamicBreaks(getShiftConfig(state.now));
  ...
  console.log(`   Tasks: ${state.taskLog.length} | Analysts (weekday): ${SHIFT_CONFIG.weekday.length}`);
}
